/*----------------------------------------------------------------------
Program hmd.c performs parallel molecular-dynamics for Lennard-Jones 
systems using hybrid MPI+OpenMP programming.
----------------------------------------------------------------------*/
#include "hmd.h"
...
----------------------------------------------------------------------*/
  ...
  for (a=0; a<3; a++) {
    lc[a] = al[a]/RCUT;
    /* HMD: size of the cell block that each thread is assigned */
    thbk[a] = lc[a]/vthrd[a];
    /* # of linked-list cells are integer multiples of the # of threads */
    lc[a] = thbk[a]*vthrd[a];
    rc[a] = al[a]/lc[a];
  }
  if (sid == 0) {
...
----------------------------------------------------------------------*/
  ...
  double lpe,rrCut;
  /* HMD: critical variable for thread reduction */
  double lpe_td[nthrd];

  /* Reset the potential & forces */
  lpe = 0.0;
  for (i=0; i<n; i++) for (a=0; a<3; a++) ra[i][a] = 0.0;
  /* HMD: reset the critical variable */
  ...
  /* HMD: SPMD parallelism by linked-list cell blocks */
#pragma omp parallel private(mc,c,mc1,c1,i,j,a)
  {
    double dr[3],rr,ri2,ri6,r1,fcVal,f,vVal;
    /* std|vtd[] = scalar|vector thread index; mofst[] = cell-block offset */
    int std,vtd[3],mofst[3];

    /* HMD: scalar & vector index of this thread */
    std = omp_get_thread_num();
    vtd[0] = std/(vthrd[1]*vthrd[2]);
    vtd[1] = (std/vthrd[2])%vthrd[1];
    vtd[2] = std%vthrd[2];
    /* HMD: linked-list cell-index offset of the cell block */
    for (a=0; a<3; a++)
      mofst[a] = vtd[a]*thbk[a];

    /* HMD: scan center cells within the cell block this thread is assigned */
        ...
        while (i != EMPTY) {
          ...
          while (j != EMPTY) {
            ...
            if (j != i) {
              ...
              /* Calculate potential & forces for all pairs if rij < RCUT */
              if (rr<rrCut) {
                ri2 = 1.0/rr; ri6 = ri2*ri2*ri2; r1 = sqrt(rr);
                fcVal = 48.0*ri2*ri6*(ri6-0.5) + Duc/r1;
                vVal = 4.0*ri6*(ri6-1.0) - Uc - Duc*(r1-RCUT);
                /* HMD: avoid critical section using separate accumulators */
                lpe_td[std] += 0.5*vVal;
                for (a=0; a<3; a++) {
                  f = fcVal*dr[a];
                  ra[i][a] += f;
  ...
  } /* HMD: end OpenMP parallel block */

  /* HMD: thread reduction of the critical variable */
  for (i=0; i<nthrd; i++)
    lpe += lpe_td[i];
